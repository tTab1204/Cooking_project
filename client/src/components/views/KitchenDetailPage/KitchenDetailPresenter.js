...
function KitchenDetailPresenter({ DetailKitchen, ShowSuccess, successMessage }) {
  const [ShowModal, setShowModal] = useState(false);

  const { images, name, address, capacity, description, rent_price } = DetailKitchen;
                  ...
                  images.map((image, index) => (
                    <Col key={index} className='gutter-row' lg={6} md={12} sm={24}>
                      <Card
                        ...
                        bodyStyle={{ padding: '0' }}
                        style={imageStyle}
                        cover={<img alt='kitchen-detail-images' src={`${LOCAL_SERVER}${image}`} />}
                      ></Card>
                    </Col>
                  ))}
              </Row>
            ...
            <PriceWrapper>
              ...
              {!ShowSuccess && (
                <Row gutter={[12]}>
                  ...
                  <Button type='primary' size='large' style={buttonStyle} onClick={onHandleModal}>
                    Contact
                  </Button>
                </Row>
              )}
      ...
      >
        <Form layout='vertical'>
          ...
          <Form.Item label='Email' required>
            ...
            <Button type='primary' size='large' style={{ width: '100%' }} onClick={onSubmit}>
              Submit
            </Button>
          </Form.Item>
        </Form>
