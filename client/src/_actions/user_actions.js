...
export const showCartItems = (cartItems, userCart) => {
  const request = axios.get(`${EVENTS_SERVER}/events_by_id?id=${cartItems}&type=array`).then((response) => {
    let data = response.data;

    userCart.forEach((cartItem) => {
      data.forEach((eventDetail, i) => {
        if (cartItem.id === eventDetail._id) data[i].quantity = cartItem.quantity;
      });
    });
    return data;
  });
...
export const removeCartItem = (eventId) => {
  ...
  const request = axios.post(`${USER_SERVER}/remove-cart-item`, variable).then((response) => {
...
export const onSuccessPay = (cartData) => {
  const request = axios.post(`${USER_SERVER}/payment-success`, cartData).then((response) => response.data);
